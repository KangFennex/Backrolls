...
export default function KikiPageClient() {
    const { navigateToBackroll } = useNavigationContext();
    const { data: kikiData } = useQuotesByCommentCount(20);
    ...
    return (
        <PageComponentContainer title="Kiki Backrolls" subtitle="Talk of the Town!">
            {kikiData?.quotes && kikiData.quotes.map((quote: Quote, index: number) => (
                <div key={quote.id}>
